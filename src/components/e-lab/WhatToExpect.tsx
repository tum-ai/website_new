    ...
    <Section className="relative w-full bg-white py-12 sm:py-12 lg:py-16">
      ...
      <div className="relative mx-auto max-w-4xl px-6">
        ...
        <div className="w-full">
          <h2 className={`text-center text-3xl md:text-4xl font-semibold uppercase tracking-tight text-black }`}
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className={`mt-1 text-sm leading-relaxed text-gray-700 `}
            style={{ fontFamily: archivo }}>{body}</p>
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <div ref={ref} className="text-center">
      ...
      <div className={`mt-2 text-sm text-gray-600`}
        style={{ fontFamily: archivo }}>{title}</div>
    </div>
  );
}

function AnimatedText({ value, prefix = "", suffix = "", decimals = 0 }: { value: MotionValue<number>; prefix?: string; suffix?: string; decimals?: number }) {
