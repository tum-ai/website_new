...
export default function FAQ({ munichFaqs }: FaqProps) {
    ...
    return (
        <section className="container mx-auto max-w-4xl px-2 py-16">
            ...
            <div className="space-y-6">
                {munichFaqs.map((faq, index) => (
                    <div key={index} className="border-b-2 border-gray-300 pb-4">
                        <div
                            className="flex justify-between items-center cursor-pointer"
                            onClick={() => setOpenFaqIndex(openFaqIndex === index ? null : index)}
                        >
                            <p className={`text-lg font-semibold ${openFaqIndex === index ? 'text-purple-700' : 'text-gray-800'}`}>{faq.question}</p>
                            <svg
